...
There exists a natural number system (Dedeking-Pierce Object).
\end{axiom}
...
Further it follows that if an object $A$ has precisely one element then $A = 1$ (TODO)
\end{remark}

\begin{axiom}[AC]
For any morphism $f$, if there exists an $x \in dom(f)$ then there exists a morphism $g$ such that $fgf = f$.
\end{axiom}
...
We write $a \subseteq b$ if for some $A$, $a$ and $b$ are both subsets of $A$ and there exists $h$ such that $a = hb$.
\end{definition}
...
\begin{axiom}[ToDO]
Each element of a sum is a member of one of the injections.
\end{axiom}

\begin{axiom}[Existence of Larger Sets]
